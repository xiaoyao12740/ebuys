# EBuy 电商平台 / EBuy E-Commerce Platform
...
<p align="center"><img alt="Tech stack" src="https://img.shields.io/badge/stack-Java%20Web-2563eb"> <img alt="Documentation language" src="https://img.shields.io/badge/docs-中文%20%7C%20English-16a34a"> <img alt="Project status" src="https://img.shields.io/badge/status-historical--learning-6b7280"></p>
...
基于 Spring MVC、MyBatis 与 JSP 的 Java Web 商城，覆盖用户、商品、新闻、留言和后台分层。 本仓库强调“问题—方法—代码—结果”的对应关系，适合作为课程作业、算法练习或建模研究的可复现档案。

### 核心方法

- Controller–Service–Mapper 分层 / layered MVC architecture
- MyBatis 数据访问 / MyBatis persistence
- JSP、JavaScript 与静态资源页面 / JSP-based views and front-end assets

## English

### Positioning

A Java web storefront built with Spring MVC, MyBatis, and JSP, covering users, products, news, messages, and layered administration logic. The repository keeps the problem statement, implementation, and outputs close together so the work can be inspected and reproduced.
...
| --- | --- | --- |
| 输入 / Input | 整理 `浏览器 / Browser` 及模型所需参数，检查单位、范围和文件位置。 | Prepare the 浏览器 / Browser and validate units, ranges, and file locations. |
| 处理 / Process | 通过 `Spring MVC` 将原始问题转换为可计算表示。 | Convert the original problem into a computable representation through Spring MVC. |
| 求解 / Solve | 执行 `Service`，保留中间结果以便检查。 | Execute Service and retain intermediate artifacts for inspection. |
| 输出 / Output | 生成 `MyBatis / Database`，并结合约束解释结果。 | Produce MyBatis / Database and interpret it together with the constraints. |

## 方法设计 / Method Design

| # | 方法与作用 / Method and role |
| ---: | --- |
| 1 | Controller–Service–Mapper 分层 / layered MVC architecture |
| 2 | MyBatis 数据访问 / MyBatis persistence |
| 3 | JSP、JavaScript 与静态资源页面 / JSP-based views and front-end assets |
...
```mermaid
flowchart LR
    A["浏览器 / Browser"] --> B["Spring MVC"] --> C["Service"] --> D["MyBatis / Database"]
```
...
源码和 Maven 工程已完整下载；运行需要匹配的 JDK、Maven、Servlet 容器及本地数据库配置。 / The Maven project is complete; runtime verification requires a compatible JDK, Maven, servlet container, and local database configuration.
...
| 路径 / Path | 作用 / Purpose |
| --- | --- |
| `pom.xml` | Maven 依赖与构建 / Maven build |
| `src/main/java/` | 控制器、服务、Mapper、实体 / application layers |
| `src/main/webapp/` | JSP 与前端资源 / views and assets |
| `src/main/resources/` | 配置与映射 / configuration |
...
```text
mvn clean package
# Deploy the generated WAR to a compatible servlet container and configure the database connection.
```
